...
本引擎在 TCP 层面采取 **轻量级转发策略**：接收方向不做乱序重组，依赖通信对端的内核协议栈（Dup-ACK 触发快速重传）保证可靠性；发送方向维护最精简的 RTO 重传（无重传队列，直接重读用户写缓冲）与零窗口持久探测，避免在丢包链路上发送侧永久挂起。这种设计在保持低 CPU 开销与内存占用的同时，保证了虚拟连接在真实网络环境下的基本可靠性。
...
本引擎在 TCP 处理上采取轻量级转发策略：接收方向不缓存乱序段，以最低开销完成 L3/L4 拦截；发送方向提供 RTO 重传与零窗口持久探测，保证写操作在确认前不丢失数据。
...
4. **极低的协议开销**：接收方向放弃乱序重组，通过 Dup-ACK 触发客户端快速重传；发送方向仅以 RTO 计时器 + 重读用户缓冲实现重传，无重传队列与额外拷贝，实现低 CPU 占用的高速转发。
