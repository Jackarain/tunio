...
协议栈采用轻量级转发策略：接收方向不做乱序重组，依赖对端内核协议栈保证
可靠性；发送方向以最精简的 RTO 重传（直接重读用户写缓冲，无重传队列）与
零窗口持久探测保证写操作在丢包链路上仍能完成，从而兼顾低 CPU 开销与基本
可靠性。所有内部状态变更强制运行于单个 Strand 之上，支持多线程运行
`io_context` 且无锁竞争。
...
- **TCP/UDP 对称抽象**：TCP 提供 `tun_tcp_socket`/`tun_tcp_acceptor`，UDP 提供
  ...
  降低学习成本。
- **极低协议开销**：接收方向放弃乱序重组，通过 Dup-ACK 触发客户端快速
  重传；发送方向仅以 RTO 计时器重读用户缓冲实现重传，无重传队列与拷贝。
