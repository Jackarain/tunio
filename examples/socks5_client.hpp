...
inline net::awaitable<net::ip::tcp::socket> socks5_connect(net::ip::tcp::endpoint proxy,
               std::string target_host,
               uint16_t target_port) {
 auto ex = co_await net::this_coro::executor;
 net::ip::tcp::socket sock(ex);
 error_code ec;
 co_await sock.async_connect(proxy, net::redirect_error(net::use_awaitable, ec));
 detail::throw_if(ec, "socks5: connect proxy");

 // 版本协商：5 / 1 种方法 / NO AUTH
 std::array<uint8_t, 3> hello{5, 1, 0};
 co_await net::async_write(sock, net::buffer(hello), net::redirect_error(net::use_awaitable, ec));
 detail::throw_if(ec, "socks5: send greeting");
 std::array<uint8_t, 2> resp{};
 co_await net::async_read(sock, net::buffer(resp), net::redirect_error(net::use_awaitable, ec));
 detail::throw_if(ec, "socks5: read method");
 if (resp[1] != 0) {
  throw boost::system::system_error(net::error::fault, "socks5: auth method not accepted");
 }
 ...
 co_await net::async_write(sock, net::buffer(req), net::redirect_error(net::use_awaitable, ec));
 detail::throw_if(ec, "socks5: send connect");

 // 解析回复：头部 4 字节 + 按 ATYP 读取剩余
 std::array<uint8_t, 4> head{};
 co_await net::async_read(sock, net::buffer(head), net::redirect_error(net::use_awaitable, ec));
 detail::throw_if(ec, "socks5: read reply");
 if (head[1] != 0) {
  throw boost::system::system_error(net::error::connection_refused, "socks5: connect failed");
 }
 size_t rest = 0;
 switch (head[3]) {
 ...
 case detail::atyp_domain: {
  ...
  co_await net::async_read(sock, net::buffer(len), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5: read domain len");
  rest = len[0] + 2;
  break;
 }
 default:
  throw boost::system::system_error(net::error::fault, "socks5: bad atyp");
 }
 if (rest > 0) {
  std::vector<uint8_t> tmp(rest);
  co_await net::async_read(sock, net::buffer(tmp), net::redirect_error(net::use_awaitable, ec));
...
public:
 explicit socks5_udp_relay(net::any_io_executor ex) : sock_(ex) {}

 // 发起 UDP ASSOCIATE 并绑定中继端点；控制连接随对象存活而保持
 net::awaitable<net::ip::udp::endpoint> associate(net::ip::tcp::endpoint proxy) {
  auto ex = co_await net::this_coro::executor;
  auto control = std::make_shared<net::ip::tcp::socket>(ex);
  error_code ec;
  co_await control->async_connect(proxy, net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: connect proxy");

  std::array<uint8_t, 3> hello{5, 1, 0};
  co_await net::async_write(*control, net::buffer(hello), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: greeting");
  std::array<uint8_t, 2> resp{};
  co_await net::async_read(*control, net::buffer(resp), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: method");
  if (resp[1] != 0) {
   throw boost::system::system_error(net::error::fault, "socks5-udp: auth method not accepted");
  }

  // UDP ASSOCIATE：BND.ADDR 为 127.0.0.1:0（0.0.0.0:0 亦可）
  std::array<uint8_t, 10> req{5, 3, 0, 1, 127, 0, 0, 1, 0, 0};
  co_await net::async_write(*control, net::buffer(req), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: associate");

  std::array<uint8_t, 4> head{};
  co_await net::async_read(*control, net::buffer(head), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: reply");
  if (head[1] != 0) {
   throw boost::system::system_error(net::error::connection_refused, "socks5-udp: associate failed");
  }
  ...
  case detail::atyp_ipv4: {
   ...
   co_await net::async_read(*control, net::buffer(rest), net::redirect_error(net::use_awaitable, ec));
   detail::throw_if(ec, "socks5-udp: bnd addr");
   net::ip::address_v4::bytes_type b{rest[0], rest[1], rest[2], rest[3]};
   relay = {net::ip::address_v4(b), static_cast<uint16_t>((rest[4] << 8) | rest[5])};
   break;
  }
  case detail::atyp_domain: {
   std::array<uint8_t, 1> len{};
   co_await net::async_read(*control, net::buffer(len), net::redirect_error(net::use_awaitable, ec));
  ...
  case detail::atyp_ipv6: {
   ...
   relay = {net::ip::address_v6(b), static_cast<uint16_t>((rest[16] << 8) | rest[17])};
   break;
  }
  default:
   throw boost::system::system_error(net::error::fault, "socks5-udp: bad atyp");
  }

  // UDP 套接字绑定中继端点（connect 后仅与中继通信，符合 SOCKS5 语义）
  co_await sock_.async_connect(relay, net::redirect_error(net::use_awaitable, ec));
 ...
 // 封装并发送数据报
 net::awaitable<void> send_to(std::vector<uint8_t> payload, net::ip::udp::endpoint target) {
  ...
  error_code ec;
  co_await sock_.async_send(net::buffer(pkt), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: send");
 }

 // 接收并剥离封装，返回 (payload, 原始目标地址)
 net::awaitable<std::pair<std::vector<uint8_t>, net::ip::udp::endpoint>> receive_from() {
  std::array<uint8_t, 2048> buf{};
  error_code ec;
  const size_t n = co_await sock_.async_receive(net::buffer(buf), net::redirect_error(net::use_awaitable, ec));
  detail::throw_if(ec, "socks5-udp: receive");
  const uint8_t* p = buf.data();
  if (n < 4 || p[0] != 0 || p[1] != 0 || p[2] != 0) {
   throw boost::system::system_error(net::error::fault, "socks5-udp: bad header");
  }
  size_t off = 3;
  ...
  case detail::atyp_ipv4: {
   if (n < off + 6) {
    throw boost::system::system_error(net::error::fault, "socks5-udp: short packet");
   }
   net::ip::address_v4::bytes_type b{p[off], p[off + 1], p[off + 2], p[off + 3]};
   off += 4;
   target = {net::ip::address_v4(b), static_cast<uint16_t>((p[off] << 8) | p[off + 1])};
   off += 2;
   break;
  }
  case detail::atyp_domain: {
   ...
   off += dlen;
   target = {net::ip::make_address(host), static_cast<uint16_t>((p[off] << 8) | p[off + 1])};
   off += 2;
   break;
  }
  case detail::atyp_ipv6: {
   ...
   off += 16;
   target = {net::ip::address_v6(b), static_cast<uint16_t>((p[off] << 8) | p[off + 1])};
   off += 2;
   break;
  }
  default:
