...
void* operator new(std::size_t n) {
    if (g_alloc) g_alloc->fetch_add(1, std::memory_order_relaxed);
...
inline uint16_t big16(const uint8_t* p) { return static_cast<uint16_t>((p[0] << 8) | p[1]); }
...
inline std::vector<uint8_t> make_tcp(uint32_t src, uint32_t dst, uint16_t sport, uint16_t dport,
    ...
    const uint32_t pseudo = (src >> 16) + (src & 0xffff) + (dst >> 16) + (dst & 0xffff) + 6 + seg.size();
...
inline std::vector<uint8_t> make_udp(uint32_t src, uint32_t dst, uint16_t sport, uint16_t dport,
    ...
    const uint32_t pseudo = (src >> 16) + (src & 0xffff) + (dst >> 16) + (dst & 0xffff) + 17 + seg.size();
    uint16_t c = csum16(seg.data(), seg.size(), pseudo);
...
// 修改 seq/ack 字段后重算 IPv4 TCP 校验和（seg 为完整 IP 包，源/目的为 host order）
inline void refresh_tcp_checksum(std::vector<uint8_t>& seg, uint32_t src, uint32_t dst) {
    seg[36] = 0;
    seg[37] = 0;
    const size_t tcp_len = seg.size() - 20;
    const uint32_t pseudo = (src >> 16) + (src & 0xffff) + (dst >> 16) + (dst & 0xffff) + 6 + tcp_len;
...
void measure(const char* name, long long n, long long warmup, F&& f) {
    for (long long i = 0; i < warmup; i++) f();
...
tun_stream establish_tcp(engine_env& env, tun_acceptor& acc, uint32_t& engine_iss) {
    tun_stream peer(env.io.get_executor());
    latch done;
    acc.async_accept(peer, [&](boost::system::error_code) { done.post(); });
    env.dev.send(make_tcp(CLIENT_IP, DEST_IP, CLIENT_PORT, DEST_PORT, 0x02, 1000, 0, 65535, {}, true));
...
static void bench_tcp_read(long long n, long long warmup) {
    ...
    char buf[64];
    auto seg = make_tcp(CLIENT_IP, DEST_IP, CLIENT_PORT, DEST_PORT, 0x18, 1001, engine_iss + 1, 65535,
                        std::vector<uint8_t>(16, 0x61));
    uint32_t seq = 1001;
    measure("tcp_read_some", n, warmup, [&] {
        peer.async_read_some(net::buffer(buf), [&](boost::system::error_code, size_t) { done.post(); });
...
static void bench_tcp_write(long long n, long long warmup) {
    ...
    auto ack_seg = make_tcp(CLIENT_IP, DEST_IP, CLIENT_PORT, DEST_PORT, 0x10, 1001, 0, 65535, {});
    std::vector<uint8_t> pkt;
    char wbuf[16];
    std::memset(wbuf, 0x62, sizeof(wbuf));
    uint32_t client_ack = engine_iss + 1;
    measure("tcp_write_some", n, warmup, [&] {
        peer.async_write_some(net::buffer(wbuf), [&](boost::system::error_code, size_t) { done.post(); });
        done.wait();
        if (!env.dev.read_packet(pkt)) {
            throw std::runtime_error("no tcp data segment");
        }
        const uint32_t pkt_seq = big32(pkt.data() + 24);
        const uint32_t pkt_len = big16(pkt.data() + 2) - 40; // IP total - 20(IP) - 20(TCP)
...
static void bench_udp(long long n, long long warmup) {
    ...
    env.dev.send(make_udp(CLIENT_IP, DEST_IP, CLIENT_PORT, DEST_PORT, std::vector<uint8_t>(16, 0x63)));
    done.wait();

    auto udp_seg = make_udp(CLIENT_IP, DEST_IP, CLIENT_PORT, DEST_PORT, std::vector<uint8_t>(16, 0x63));
    std::vector<uint8_t> pkt;
    char buf[64];
    char sbuf[16];
    ...
    measure("udp_receive", n, warmup, [&] {
        sock.async_receive(net::buffer(buf), [&](boost::system::error_code, size_t) { done.post(); });
        env.dev.send(udp_seg);
        done.wait();
    });

    measure("udp_send", n, warmup, [&] {
        sock.async_send(net::buffer(sbuf), [&](boost::system::error_code, size_t) { done.post(); });
        done.wait();
        if (!env.dev.read_packet(pkt)) {
...
static void bench_accept(long long n, long long warmup) {
    engine_env env;
    tun_acceptor acc(env.engine);
    auto syn = make_tcp(CLIENT_IP, DEST_IP, CLIENT_PORT, DEST_PORT, 0x02, 1000, 0, 65535, {}, true);
    std::vector<uint8_t> synack;
    ...
    measure("accept", n, warmup, [&] {
        ...
        auto ack = make_tcp(CLIENT_IP, DEST_IP, cport - 1, DEST_PORT, 0x10, 1001, engine_iss + 1, 65535, {});
        env.dev.send(ack);
        done.wait();
    });
}
