...
static void test_session_recreated_after_expiry() {
    ...
    // 第一个会话
    {
        ...
        acceptor.async_accept(s1, [&](boost::system::error_code ec) { a1.set_value(ec); });
    ...
    acceptor.async_accept(s2, [&](boost::system::error_code ec) { a2.set_value(ec); });
...
int main(int argc, char** argv) {
    if (argc > 1 && std::string(argv[1]) == "roundtrip") { test_datagram_roundtrip(); return 0; }
    if (argc > 1 && std::string(argv[1]) == "timeout") { test_session_timeout(); return 0; }
    if (argc > 1 && std::string(argv[1]) == "recreate") { test_session_recreated_after_expiry(); return 0; }
