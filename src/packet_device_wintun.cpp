...
boost::system::error_code win_last_error() {
 ...
 DWORD n = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM |
  FORMAT_MESSAGE_IGNORE_INSERTS, nullptr, err, 0, buf, sizeof(buf), nullptr);
...
static std::string narrow_from_wide(const wchar_t* ws) {
 ...
 WideCharToMultiByte(CP_ACP, 0, ws, -1, &result[0], static_cast<int>(result.size()), nullptr, nullptr);
...
static std::wstring wide_from_narrow(const char* ns) {
 ...
 MultiByteToWideChar(CP_ACP, 0, ns, -1, &result[0], static_cast<int>(result.size()));
...
void fill_connection_names(std::vector<windows_driver_info>& result) {
 ...
 HKEY key;
 if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, net_connections_key, 0,
...
enum_net_device_interfaces() {
  ...
  for (DWORD j = 0; SetupDiEnumDeviceInfo(h_devs, j, &dev_info); ++j) {
   ...
   if (SetupDiEnumDeviceInterfaces(h_devs, &dev_info, &iface_guid, j,
    ...
    auto* detail = reinterpret_cast<
     SP_DEVICE_INTERFACE_DETAIL_DATA_W*>(buffer.get());
    detail->cbSize =
     sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA_W) + 2;
    if (SetupDiGetDeviceInterfaceDetailW(
      h_devs, &iface_data, detail, required, &required,
      nullptr)) {
     ...
     if (reg_hk != INVALID_HANDLE_VALUE) {
      wchar_t nid[256] = { 0 };
      DWORD t = 0;
      DWORD l = static_cast<DWORD>(
       sizeof(nid) / sizeof(wchar_t));
      if (RegQueryValueExW(
        reg_hk, L"NetCfgInstanceId", nullptr, &t,
 ...
 HDEVINFO h_all = SetupDiGetClassDevsExW(
  const_cast<GUID*>(&GUID_DEVCLASS_NET), nullptr, nullptr,
 ...
 for (DWORD i = 0; SetupDiEnumDeviceInfo(h_all, i, &data); ++i) {
  ...
  if (dev_key != INVALID_HANDLE_VALUE) {
   ...
   if (RegQueryValueExW(dev_key, L"NetCfgInstanceId", nullptr, &t,
    ...
    if (!path.empty())
     result.emplace_back(std::wstring(nid),
          std::move(path));
   }
   RegCloseKey(dev_key);
  }
...
HANDLE open_wintun_device(const std::wstring& name) {
 const wchar_t adapter_key[] =
  L"SYSTEM\\CurrentControlSet\\Control\\Class\\"
  L"{4D36E972-E325-11CE-BFC1-08002BE10318}";
 std::vector<windows_driver_info> devs;

 HKEY key;
 if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, adapter_key, 0, KEY_READ,
      &key) == ERROR_SUCCESS) {
  for (int i = 0;; ++i) {
   wchar_t enum_name[256] = { 0 };
   DWORD len = static_cast<DWORD>(
    sizeof(enum_name) / sizeof(wchar_t));
   if (RegEnumKeyExW(key, i, enum_name, &len, nullptr, nullptr,
        nullptr, nullptr) != ERROR_SUCCESS)
    break;
   ...
   HKEY unit_key;
   if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, unit.c_str(), 0,
   ...
   bool has_comp = RegQueryValueExA(unit_key, "ComponentId",
           nullptr, &type,
   ...
   if (has_comp) {
    ...
    has_inst = RegQueryValueExA(unit_key, "NetCfgInstanceId",
           nullptr, &type,
...
bool register_rings_ioct(HANDLE dev_handle, struct tun_ring* send_ring,
 ...
 return DeviceIoControl(
    dev_handle, TUN_IOCTL_REGISTER_RINGS, &rr, sizeof(rr), nullptr,
    0, &bytes_returned, nullptr) != FALSE;
}
...
int wintun_packet_device_impl::recv_one(std::string_view buf) {
 ...
 ULONG aligned = wintun_ring_packet_align(
  sizeof(struct TUN_PACKET_HEADER) + pkt->size);
...
void wintun_packet_device_impl::cleanup_rings() {
 if (send_ring_) { UnmapViewOfFile(send_ring_); send_ring_ = nullptr; }
 if (recv_ring_) { UnmapViewOfFile(recv_ring_); recv_ring_ = nullptr; }
 if (send_evt_) { CloseHandle(send_evt_); send_evt_ = INVALID_HANDLE_VALUE; }
 if (recv_evt_) { CloseHandle(recv_evt_); recv_evt_ = INVALID_HANDLE_VALUE; }
 if (send_ring_fh_) {
...
bool wintun_packet_device_impl::open(const device_config& cfg,
 ...
 static const GUID adapter_guid = {
  0xDEADBA11, 0xCAFE, 0xBEEF, { 0x01, 0x23, 0x45, 0x67,
          0x00, 0x00, 0x00, 0xFF }
 };
 ...
 send_ring_fh_ = CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr,
          PAGE_READWRITE, 0,
          sizeof(struct tun_ring), nullptr);
 recv_ring_fh_ = CreateFileMappingW(INVALID_HANDLE_VALUE, nullptr,
          PAGE_READWRITE, 0,
          sizeof(struct tun_ring), nullptr);
