...
bool add_ipv6_address(int ioctl_sock, const std::string& ifname, const std::string& addr,
    ...
    for (;;) {
        ...
        if (r <= 0) {
            ec = boost::system::error_code(ETIMEDOUT, boost::system::generic_category());
            ::close(nl);
            return false;
        }
        char rbuf[4096];
        ...
        for (struct nlmsghdr* h = reinterpret_cast<struct nlmsghdr*>(rbuf);
            ...
            ec = boost::system::error_code(-err->error, boost::system::generic_category());
            ::close(nl);
            return false;
        }
    }
...
bool posix_packet_device_impl::open(const device_config& cfg, boost::system::error_code& ec) {
    ...
    // 配置 IPv6 地址（可选）
    if (!cfg.ipv6.empty() && !add_ipv6_address(s, cfg.name, cfg.ipv6, cfg.ipv6_prefix_len, ec)) {
        ::close(s);
        ::close(fd);
        return false;
    }
